...
def create_legs(i):
    ...
    tibia_frame = compound([cylinder(pos=vector(0, 0, 0), length=tibia_len, radius=6, color=color.blue)], pos=vector(femur_len, 0, 0), axis=vector(0, 0, -1))
    
    femur_frame = compound([tibia_frame, cylinder(pos=vector(0, 0, 0), length=femur_len, radius=6, color=color.green)], pos=vector(coxa_len, 0, 0), axis=vector(1, 0, 0))
    
    coxa_frame = compound([femur_frame, cylinder(pos=vector(0, 0, 0), length=coxa_len, radius=6, color=color.red)], pos=vector(0, 0, 0), axis=vector(1, 0, 0))
...
def draw_legs(leg, a, b, g):
    ...
    coxa[leg].axis = vector(x_dir * cos(g), y_dir * sin(g), 0)
    femur[leg].axis = vector(cos(a), 0, sin(a))
    tibia[leg].axis = vector(-cos(b), 0, -sin(b))
...
for xx in range(-200, 200, 10):
    ...
    curve(pos=[vector(xx, -200, z_stand_draw), vector(xx, 200, z_stand_draw)], color=color_t)
...
for yy in range(-200, 200, 10):
    if (yy % 50):
        ...
        color_t = color.yellow
    curve(pos=[vector(-200, yy, z_stand_draw), vector(200, yy, z_stand_draw)], color=color_t)

curve(pos=[vector(-200, 200, z_stand_draw), vector(200, 200, z_stand_draw)], color=color.yellow)
curve(pos=[vector(200, -200, z_stand_draw), vector(200, 200, z_stand_draw)], color=color.yellow)
...
curve(pos=[vector(-x_len, y_len, z_stand_draw), vector(x_len, y_len, z_stand_draw),
           vector(x_len, -y_len, z_stand_draw), vector(-x_len, -y_len, z_stand_draw),
           vector(-x_len, y_len, z_stand_draw)], color=color.red)

'''
    Create body & legs
'''
body = box(pos=vector(0, 0, body_z / 2), length=body_x, height=body_y, width=body_z,
           color=color.magenta)
